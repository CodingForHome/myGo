...
func testPeriodLimit(t *testing.T, opts ...PeriodOption) {
	// 声明一个redis
	redis := redis.RedisConf{Host: "0.0.0.0:6379", Pass: "123456"}.NewRedis()
	const (
		seconds = 1
		total   = 100
		quota   = 5
	)
	l := NewPeriodLimit(seconds, quota, redis, "periodlimit", opts...)
...
func TestRedis(t *testing.T) {
	// 声明一个redis
	redis := redis.RedisConf{Host: "0.0.0.0:6379", Pass: "123456"}.NewRedis()
	err := redis.Set("name", "jinfeng")
	if err != nil {
		fmt.Println(err.Error())
		return
	}
	println(redis.Get("periodlimitfirst"))
}
